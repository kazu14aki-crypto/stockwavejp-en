...
export default function LegalConsentGate({onNavigate}){
  ...
  const save=async()=>{
    ...
    if(error){setSaving(false);setError('The consent record could not be saved. Check that the Supabase migration has been applied.');return}
  ...
  return <div style={{position:'fixed',inset:0,zIndex:5000,display:'grid',placeItems:'center',padding:'18px',background:'rgba(3,7,15,.84)',backdropFilter:'blur(6px)'}}><div style={{width:'min(560px,100%)',background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:'14px',padding:'22px'}}><h2 style={{margin:'0 0 8px',fontSize:'20px',color:'var(--text)'}}>Accept the legal terms</h2><p style={{fontSize:'12px',lineHeight:1.8,color:'var(--text2)'}}>Review and accept the following documents before using your account.</p><div style={{display:'flex',gap:'12px',flexWrap:'wrap',margin:'12px 0'}}><button style={linkStyle} onClick={()=>window.open(location.origin+'#terms','_blank','noopener')}>Terms of Service</button><button style={linkStyle} onClick={()=>window.open(location.origin+'#privacy','_blank','noopener')}>Privacy Policy</button><button style={linkStyle} onClick={()=>window.open(location.origin+'#disclaimer','_blank','noopener')}>Disclaimer</button></div><label style={{display:'flex',gap:'9px',padding:'12px',border:'1px solid var(--border)',borderRadius:'9px',fontSize:'12px',color:'var(--text2)'}}><input type="checkbox" checked={checked} onChange={e=>setChecked(e.target.checked)}/><span>I have reviewed and agree to all three documents.</span></label>{error&&<div style={{fontSize:'11px',color:'#ff647c',marginTop:'9px'}}>{error}</div>}<button disabled={!checked||saving} onClick={save} style={{width:'100%',marginTop:'14px',padding:'11px',border:0,borderRadius:'8px',background:checked?'var(--accent)':'var(--bg3)',color:checked?'#fff':'var(--text3)',fontWeight:700,cursor:checked?'pointer':'not-allowed'}}>{saving?'…':'Accept and continue'}</button></div></div>
}
