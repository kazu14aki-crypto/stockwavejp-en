...
const STEPS = [
  { icon:'①', title:'Find strong themes', text:'「テーマ一覧」で1週間を選び、表示指標を「市場超過騰落率」に切り替えて、TOPIXより強いテーマを探します。' },
  { icon:'②', title:'Test whether the move is broad', text:'「テーマ別詳細」で出来高、成績分布、構成銘柄を確認し、特定1社だけが平均を押し上げていないかを見ます。' },
  { icon:'③', title:'個別Compare stocks', text:'構成銘柄表と「銘柄検索」を使い、業績、バリュエーション、流動性、企業開示を比較します。' },
  { icon:'④', title:'Track persistence', text:'「レポート」の週次レポートで前回ランキングの事後成績を確認し、テーマが継続しているか失速したかを追います。' },
]
  ...
  return <div role="dialog" aria-modal="true" aria-label="StockWaveJP初回ガイド" style={{position:'fixed',inset:0,zIndex:3000,background:'rgba(4,8,16,.78)',display:'grid',placeItems:'center',padding:'16px'}}>
    <div style={{width:'min(560px,100%)',background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:'14px',padding:'24px',boxShadow:'0 24px 80px rgba(0,0,0,.45)'}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',gap:'8px'}}>
        <button onClick={close} style={buttonStyle('transparent','var(--text3)')}>Close</button>
        <div style={{display:'flex',gap:'8px'}}>
          {step>0 && <button onClick={()=>setStep(step-1)} style={buttonStyle('var(--bg3)','var(--text2)')}>Back</button>}
          {step<STEPS.length-1
            ? <button onClick={()=>setStep(step+1)} style={buttonStyle('var(--accent)','#fff')}>Next</button>
            : <button onClick={()=>{ close(); onNavigate?.('Theme List') }} style={buttonStyle('var(--accent)','#fff')}>テーマ一覧を見る</button>}
        </div>
      </div>
    </div>
  </div>
