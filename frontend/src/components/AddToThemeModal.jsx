      ...
      <div style={{
        ...
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:'14px' }}>
          ...
          <button onClick={onClose} style={{ background:'none', border:'none', color:'var(--text3)',
            cursor:'pointer', fontSize:'16px', padding:'0 0 0 8px', lineHeight:1 }}>✕</button>
        </div>
        {done ? (
          <div style={{ textAlign:'center', padding:'16px 0', fontSize:'13px', color:'var(--green)', fontWeight:600 }}>
            ✓ {msg}
          </div>
        ) : (
          <>
            <div style={{ display:'flex', gap:'4px', marginBottom:'14px' }}>
              {[['select','Add to existing'],['create','Create new']].map(([k,l]) => (
            ...
            {mode === 'select' ? (
              themes.length === 0 ? (
                <div style={{ textAlign:'center', padding:'16px', fontSize:'12px', color:'var(--text3)' }}>
                  No custom themes yet.<br/>Use "Create new" to add one.
                </div>
              ) : (
              ...
              <div>
                <div style={{ fontSize:'11px', color:'var(--text3)', marginBottom:'6px' }}>New theme name</div>
                <div style={{ display:'flex', gap:'6px' }}>
                  <input value={newName} onChange={e => setNewName(e.target.value)}
                    onKeyDown={e => e.key==='Enter' && handleCreate()}
                    placeholder="e.g. My Portfolio, Watchlist"
                    autoFocus
                  ...
                  <button onClick={handleCreate} disabled={!newName.trim()} style={{
                    ...
                    border:'1px solid rgba(74,158,255,0.3)', opacity: newName.trim() ? 1 : 0.4,
                  }}>Create</button>
                </div>
              </div>
            )}
