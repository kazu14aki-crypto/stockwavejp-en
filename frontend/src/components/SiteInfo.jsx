    ...
    <div style={{ padding: '28px 32px 60px', maxWidth: '760px', margin: '0 auto' }}>
      ...
      <div style={{ background:'var(--bg2)', border:'1px solid var(--border)',
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))', gap:'10px' }}>
          {[
            { icon:'📡', title:'Price Momentum', desc:'🔥加速・↗転換↑・→横ばい・↘転換↓・❄️失速の5分類は当サイト独自の指標' },
            { icon:'🗺️', title:'テーマ×期間ヒートマップ', desc:'72テーマのPrice Change %を1週〜1年の複数期間で同時比較できる' },
            { icon:'📊', title:'三指標同時表示', desc:'Price Change %・Volume・Trading Valueを72テーマ分まとめてランキング表示' },
            { icon:'⚙️', title:'Custom Theme', desc:'自分だけのテーマを作成して独自の銘柄群を追跡できる' },
            { icon:'📈', title:'マクロ連動表示', desc:'日経平均・ドル円・VIX等のマクロ指標とテーマ動向を同時確認' },
            { icon:'📰', title:'Weekly Report', desc:'毎週の相場振り返りと来週の注目ポイントを編集部が解説' },
          ].map(({ icon, title, desc }) => (
...
StockWaveJPは、半導体・AI・防衛・インバウンドなど67の投資テーマについて、Price Change %・Volume・Trading Valueを自動集計し、「資金の流れ」を視覚的に把握できるダッシュボードとして開発しました。
...
投資助言や個別銘柄の推奨ではなく、「今市場は何を注目しているか」を客観的なデータで提供することを第一の目的としています。`,
        },
        {
          ...
          title: '作成の背景・理由',
          color: '#aa77ff',
          content: `株式市場では、個別銘柄の分析だけでなく「どのテーマやセクターに資金が流れているか」という大局観が投資判断に非常に重要です。しかし、テーマ別のPrice Change %・Volume・Trading Valueを一元管理できる無料ツールは国内に限られていました。
      ...
      <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'12px', padding:'22px 26px', marginBottom:'16px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'120px 1fr', gap:'12px 24px', fontSize:'13px', marginBottom:'14px' }}>
          {[
            ...
            ['目的', '日本株テーマ別のPrice Change %・Volume・Trading Valueをリアルタイムで可視化し、投資判断の参考情報を提供すること'],
        ...
        <div style={{ display:'flex', alignItems:'flex-start', gap:'20px', flexWrap:'wrap' }}>
          ...
          <div style={{ flex:1, minWidth:'200px' }}>
            ...
            <div style={{ fontSize:'13px', color:'var(--text2)', lineHeight:2 }}>
              日本株・米国株に投資する個人投資家として、テーマ株のCapital Flowを独自に追跡・分析しています。
              ...
              分析視点を大切にしています。Price Change %・Volume・Trading Value・モメンタムの4指標を組み合わせた
