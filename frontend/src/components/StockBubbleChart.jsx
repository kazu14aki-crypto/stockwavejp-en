          ...
          {filtered.filter(s => s.ticker !== hovered?.ticker).map(s => {
            ...
            return (
              <g key={s.ticker} style={{ cursor:'pointer' }}
                ...
                onClick={(e) => {
                  // Mobile: first tap = show info, second tap = navigate
                  if ('ontouchstart' in window) {
                    e.preventDefault()
                    if (hovered?.ticker === s.ticker) {
                      // Second tap on same stock → navigate
                      if (onNavigate && themeName) onNavigate('Theme Detail', themeName)
                    } else {
                      setHovered(s)
                    }
                  }
