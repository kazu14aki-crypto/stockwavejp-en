...
// ④ コラムIDの表示ラベル
const COL_LABELS = {
  'semiconductor-theme':    'Semiconductor / AI Chip Column',
  'power-semiconductor':    'Power Semiconductor Column',
  'ai-cloud-theme':         'Generative AI / Cloud Column',
  'physical-ai-edge-ai':    'Physical AI / Edge AI Column',
  'defense-theme':          'Defense / Space / Cyber Column',
  'inbound-theme':          'Inbound Consumption Column',
  'banking-finance-theme':  'Banking / Finance Column',
  'saas-dx-theme':          'SaaS / DX Column',
  'ev-green-theme':         'EV / Green Column',
  'optical-communication':  'Optical Comms Column',
  'mlcc-murata-analysis':   'Murata / MLCC Column',
  'optical-theme':          'Optical Comms Column',
  'murata-seisakusho-analysis': 'Murata Manufacturing Column',
}
...
function StockChart({ ticker, period }) {
  ...
  if (loading) return <div style={{ padding:'24px', textAlign:'center', color:'var(--text3)', fontSize:'12px' }}>Fetching data...</div>
  if (!data.length) return <div style={{ padding:'24px', textAlign:'center', color:'var(--text3)', fontSize:'12px' }}>No chart data yet (available after Infoway setup)</div>
  ...
  return (
    <div style={{ overflowX:'auto' }}>
      ...
      {volData.length > 0 && (
        <>
          <div style={{ fontSize:'10px', color:'var(--text3)', margin:'4px 0 2px' }}>Volume</div>
...
export default function StockSearch({ onNavigate }) {
  ...
  const PERIODS = [
    ...
    {v:'3mo',l:'3M'},{v:'6mo',l:'6M'},{v:'1y',l:'1Y'},
  ]
  ...
  return (
    <div style={{ padding:'20px 16px 60px', maxWidth:'900px', margin:'0 auto' }}>
      ...
      {selected ? (
        ...
        <>
          <h1 style={{ fontSize:'22px', fontWeight:700, color:'var(--text)', marginBottom:'6px' }}>🔎 Stock Search</h1>
          <p style={{ fontSize:'12px', color:'var(--text3)', marginBottom:'18px', lineHeight:1.7 }}>
            Search by stock name or ticker. View price charts, theme memberships, and related columns.
          </p>
        </>
      )}
      ...
      {!selected && !searchQ && (
        <div style={{ padding:'40px 20px', textAlign:'center', color:'var(--text3)' }}>
          ...
          <div style={{ fontSize:'13px' }}>
            {loading ? 'Loading...' : `${Object.keys(stockIndex).length}stocks available`}
          </div>
        </div>
      )}
          ...
          <div>
                ...
                <div>
                  ...
                  <div style={{ display:'flex', alignItems:'baseline', gap:'10px', flexWrap:'wrap' }}>
                    ...
                    <span style={{ fontSize:'12px', color:'var(--text3)' }}>
                      Mkt Cap: <strong style={{ color:'var(--text2)' }}>{fmt(s.market_cap)}</strong>
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div style={{ background:'var(--bg2)', border:'1px solid var(--border)',
              ...
              <p style={{ fontSize:'10px', color:'var(--text3)', marginTop:'8px' }}>
                ※ Estimated. Please verify with official ETF data.
              </p>
            </div>
