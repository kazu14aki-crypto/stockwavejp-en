...
const SEG_NAME_EN = {
  ...
  'プライム市場':'Prime Market','プライム市場（大型株）':'Prime (Large)','プライム市場（中型株）':'Prime (Mid)','プライム市場（小型株）':'Prime (Small)',
  'スタンダード市場':'Standard Market','グロース市場':'Growth Market','プライム':'Prime','スタンダード':'Standard','グロース':'Growth',
...
function PickupStocks({ stocks, period }) {
  ...
  if (sparkAccel > 3)    parts.push('Price accelerating (+' + sparkAccel.toFixed(1) + '% back-half avg).')
  else if (sparkAccel > 1) parts.push('Slightly improving (+' + sparkAccel.toFixed(1) + '% back-half avg).')
      ...
      if (parts.length === 0) parts.push('High composite score across Return, Volume, momentum, and Trade Value.')
      return parts.join('。') + '。'
    }
  ...
  return (
    <div style={{ marginBottom:'20px' }}>
      ...
      <div style={{ marginTop:'8px', padding:'8px 12px',
        ...
        Rankings above are auto-calculated.
        ...
        if (r.ok) {
          ...
          if (ss.length > 0) {
            const enriched = ss.map(s => ({
              ...s, name: getStockName(s.name, s.ticker), name: tickerMap[s.ticker?.replace('.T','')] || s.name || s.ticker,
            }))
          ...
          <div>
            <div style={{ display:'flex', gap:'6px', flexWrap:'wrap', padding:'12px 0', borderBottom:'1px solid var(--border)', marginBottom:'20px' }}>
              {(groups[activeGroup]||[]).map(seg=>{
                ...
                const shortName = strans(rawShort) || strans(seg)
                return (
