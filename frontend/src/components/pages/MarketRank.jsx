...
function StockTable({ stocks, onAddToTheme }) {
  ...
  const headers = ['Price','Return','Contribution','寄与Rank','Vol.Chg','Volume','VolumeRank','T.Value','T.ValueRank','Add']
  return (
      ...
      <table style={{ borderCollapse:'collapse', fontSize:'12px', fontFamily:'var(--font)', width:'100%' }}>
        <thead>
          <tr style={{ borderBottom:'1px solid var(--border)' }}>
            ...
            <th style={{ ...thStyle, textAlign:'left', minWidth:'120px', background:'var(--bg3)' }}> stocks名</th>
            {headers.map(h => (
...
export default function MarketRank() {
  ...
  useEffect(()=>{
    ...
    const firstSeg = (marketData.groups?.['Major Stocks'] || Object.values(marketData.groups||{})[0] || [])[0]
    ...
    <div>
      ...
      <div style={{ padding:'20px 32px 48px' }}>
        <p style={{ fontSize:'12px', color:'var(--text3)', marginBottom:'16px' }}>
          Major Stocks・Broad Market・Market SegmentごとのReturnランキングと構成 stocks詳細
        </p>
