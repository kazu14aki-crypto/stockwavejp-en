  ...
  return (
    <div style={{ overflowX:'auto', WebkitOverflowScrolling:'touch' }}>
      <table style={{ borderCollapse:'collapse', fontSize:'11px', minWidth:'600px', width:'100%' }}>
        <thead>
          <tr>
            <th style={{ textAlign:'left', padding:'6px 10px', color:'var(--text3)', fontWeight:600, position:'sticky', left:0, background:'var(--bg3)', minWidth:'120px' }}>Theme</th>
            {columns.map(col => (
...
const PERIOD_LABELS = {
  '1d':'1 Day', '5d':'1 Week', '1mo':'1 Month', '3mo':'3 Months', '6mo':'6 Months', '1y':'1 Year',
}
...
function formatCompact(value, unit='') {
  ...
  if (unit === 'JPY') {
    if (abs >= 1e12) return `${(n/1e12).toFixed(1)}T JPY`
    if (abs >= 1e8) return `${(n/1e8).toFixed(1)}100MJPY`
    if (abs >= 1e4) return `${(n/1e4).toFixed(0)}10KJPY`
    return `${Math.round(n).toLocaleString()}JPY`
  }
  if (abs >= 1e12) return `${(n/1e12).toFixed(1)}T${unit}`
  if (abs >= 1e8) return `${(n/1e8).toFixed(1)}100M${unit}`
  if (abs >= 1e4) return `${(n/1e4).toFixed(0)}10K${unit}`
...
function TrendChart({ title, rows, suffix='', compactUnit='', tone='accent' }) {
  ...
  if (!values.length) {
    return (
      <div className="heatmap-trend-card">
        ...
        <div className="heatmap-empty-chart">Trend data is unavailable</div>
      </div>
    )
  }
  ...
  return (
    <div className="heatmap-trend-card">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="heatmap-mini-chart" role="img" aria-label={`${title} trend`}>
        {[0, .5, 1].map(rate => {
    ...
    <div className="heatmap-stock-list">
      ...
      {rows.length ? rows.map((stock, index) => (
        <button key={stock.ticker || stock.name || index}
          ...
          onClick={() => stock.ticker && onNavigate?.('Stock Detail', stock.ticker)}>
      ...
      )) : <div className="heatmap-empty-list">Constituent data is unavailable</div>}
    </div>
  )
}
  ...
  if (!theme) {
    return (
      <div className="heatmap-selected-empty">
        <div>Select a theme on the heatmap</div>
        <span>Hover on desktop or tap on mobile to show trends and constituent stocks here.</span>
      </div>
    )
  }
  ...
  return (
    <section className="heatmap-selected-panel" aria-live="polite">
      <div className="heatmap-selected-header">
        <div>
          <div className="heatmap-selected-eyebrow">Selected Theme</div>
          <h2>{tn(theme)}</h2>
          <div className="heatmap-selected-sub">{PERIOD_LABELS[period] || period} theme analysis</div>
        </div>
        <div className="heatmap-selected-actions">
          <button onClick={() => onNavigate?.('Theme Detail', theme)}>Theme Detail →</button>
          {THEME_ARTICLE_MAP[theme] && (
            <button className="secondary" onClick={() => onNavigate?.('Column', THEME_ARTICLE_MAP[theme])}>Related Column →</button>
          )}
        </div>
      </div>

      <StockWaveScoreCard result={stockWaveScore} locale="en" compact />
      ...
      {loading && <div className="heatmap-panel-loading">Loading analysis data…</div>}
      {error && <div className="heatmap-panel-error">{error}</div>}

      <div className="heatmap-mobile-metric-tabs">
        {[['pct','Return'],['volume','Volume'],['trade','Trading Value']].map(([key,label]) => (
      ...
      <div className="heatmap-reading-guide">
        ...
        <span>Use the median and rising-stock count to test breadth. Rising prices with increasing volume and trading value can indicate broader capital inflow, while an average driven only by the top three suggests concentration.</span>
      </div>
    </section>
  )
}
...
function BubbleScatter({ data, mPeriod, setMPeriod, onNavigate, canAccessPeriod }) {
  ...
  const yAxisLabel = hasRealVolChg ? 'Volume Acceleration (%)' : 'Change vs Prior Week (pt)'
    ...
    return (
      <div style={{ textAlign:'center', padding:'40px 20px', color:'var(--text3)',
        ...
        <div style={{ fontSize:'11px' }}>
          The chart will appear after the next scheduled market-data update.
        </div>
      </div>
    )
  }
  ...
  // JPYサイズ（Trading Valueに比例、最小8・最大40px）
  const rS = tv => {
    if (!tv || tv === 0) return 7
  ...
  // 色（Return率）サイトのred/green変数に合わせる
  const bColor = pct => {
  ...
  const zones = [
    { label:'Hot Zone: Rising + Volume',  x:x0, y:PT,  w:PL+GW-x0, h:y0-PT,    bg:'rgba(255,83,112,0.22)', border:'rgba(255,83,112,0.60)' },
    { label:'Sell Pressure: Falling + Volume',    x:PL, y:PT,  w:x0-PL,    h:y0-PT,    bg:'rgba(0,196,140,0.18)',  border:'rgba(0,196,140,0.55)'  },
    { label:'Quiet Rising',       x:x0, y:y0,  w:PL+GW-x0, h:PT+GH-y0, bg:'rgba(255,140,66,0.15)', border:'rgba(255,140,66,0.50)' },
  ...
  const fmtL = tv => {
    if (!tv) return '-'
    if (tv >= 1e8) return (tv/1e8).toFixed(0) + '100M'
    if (tv >= 1e4) return (tv/1e4).toFixed(0) + '10K'
    ...
    <div onClick={() => setPopupTheme(null)}>
      ...
      {popupTheme && (
        ...
        >
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', fontSize:'12px' }}>
              <span style={{ color:'var(--text3)' }}>Trading Value</span>
              <span style={{ fontFamily:'var(--mono)', color:'var(--text2)' }}>
                {popupTheme.trade_value ? (popupTheme.trade_value >= 1e8
                  ? (popupTheme.trade_value/1e8).toFixed(1)+'100M'
                  : (popupTheme.trade_value/1e6).toFixed(0)+'M') : '-'}
              </span>
            </div>
          </div>
          <button
            onClick={() => { onNavigate?.('Theme Detail', popupTheme.theme); setPopupTheme(null) }}
            style={{
          ...
          {(() => {
            ...
            return (
              <button
                onClick={() => { onNavigate?.('Column', colId); setPopupTheme(null) }}
                style={{
      ...
      <div style={{ display:'flex', gap:'8px', alignItems:'center', marginBottom:'16px' }}>
        ...
        <span style={{ fontSize:'11px', color:'var(--text3)' }}>
          X = Return · Y = {yAxisLabel} · Bubble size = Trading Value
        </span>
      </div>
      ...
      <div className="scatter-zone-desc">
        {[
          { label:'🔥 Hot Zone (upper right)', desc:'Rising prices with accelerating volume', color:'#ff5370' },
          { label:'⚠️ Sell Pressure (upper left)',   desc:'Falling prices with accelerating volume',    color:'#00c48c' },
          { label:'📈 Quiet Rising (lower right)',  desc:'Rising with limited volume',    color:'#ff8c42' },
          { label:'❄️ Quiet Falling (lower left)',  desc:'Weak with limited activity',             color:'#4a9eff' },
        ].map(z => (
      ...
      <div className="heatmap-chart-shell">
        ...
        >
          ...
          {/* ゾーンラベル */}
          <text x={x0+8} y={PT+16} fontSize="11" fill="rgba(255,83,112,0.85)" fontWeight="700">🔥 Hot Zone</text>
          <text x={PL+6} y={PT+16} fontSize="11" fill="rgba(0,196,140,0.8)" fontWeight="700">⚠️ Sell Pressure</text>
          <text x={x0+8} y={PT+GH-8} fontSize="11" fill="rgba(255,140,66,0.7)" fontWeight="600">📈 Quiet Rising</text>
          <text x={PL+6} y={PT+GH-8} fontSize="11" fill="rgba(74,158,255,0.65)" fontWeight="600">❄️ Quiet Falling</text>
                ...
                >
                  ...
                  {r >= 16 && (
                    <text x={cx} y={cy+3} textAnchor="middle"
                      ...
                      {tn(d.theme).length > 12 ? tn(d.theme).slice(0,12) + '…' : tn(d.theme)}
                    </text>
                  )}
                </g>
              )
            ...
            return (
              ...
              >
                ...
                <text x={cx} y={cy+4} textAnchor="middle"
                  ...
                  {tn(d.theme).length > 14 ? tn(d.theme).slice(0,14)+'…' : tn(d.theme)}
                </text>
                  ...
                  return (
                    <g style={{ pointerEvents:'none' }}>
                      ...
                      <text x={tx+12} y={ty+74} fontSize="12" fill="#8b949e">
                        {'Trading value: ' + fmtL(d.trade_value)}
                      </text>
                    </g>
                  )
                })()}
          ...
          {/* 凡例（Trading Valueバブルサイズ） */}
          {[{tv:500e8,l:'JPY500B'},{tv:100e8,l:'JPY100B'},{tv:20e8,l:'JPY20B'}].map((item, i) => {
            const r = rS(item.tv)
        ...
        return (
          <div style={{ marginTop:'16px', padding:'14px 18px',
            ...
            <div style={{ fontSize:'11px', fontWeight:600, color:'rgba(220,80,80,0.9)',
              ...
              🔥 Hot-Zone Leaders (Rising + Volume Growth)
            </div>
