...
const COLORS = ['#ff4560','#ff8c42','#ffd166','#06d6a0','#4a9eff','#aa77ff','#ff77aa','#44dddd']
function Loading() {
  return (
...
function MultiLineChart({ trends, selected }) {
  if (!selected.length) return <div style={{ textAlign:'center', padding:'40px', color:'var(--text3)', fontSize:'13px' }}>Please select at least one theme.</div>
    ...
    <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'var(--radius)', padding:'16px', overflowX:'auto' }}>
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display:'block', minWidth:'400px' }}>
        ...
        {selected.map((theme,ti)=>{
          ...
          return pts.length?<polyline key={theme} points={pts.join(' ')} fill="none" stroke={COLORS[ti%COLORS.length]} strokeWidth="2" strokeLinejoin="round" strokeLinecap="round"/>:null
        })}
      </svg>
...
export default function Compare() {
  ...
  const [selMacro,    setSelMacro]    = useState(['Nikkei 225','S&P500','USD/JPY'])
  const [loadingT,    setLoadingT]    = useState(false)
  const [loadingM,    setLoadingM]    = useState(false)
  useEffect(()=>{
    fetch(`${API}/api/theme-names`).then(r=>r.json()).then(d=>{ setThemeNames(d.themes); setSelThemes(d.themes.slice(0,3)) }).catch(()=>{})
  },[])
  useEffect(()=>{
    if (!selThemes.length) return
    setLoadingT(true)
    fetch(`${API}/api/trends?themes=${encodeURIComponent(selThemes.join(','))}&period=${period}`)
      .then(r=>r.json()).then(d=>setThemeTrends(d.trends)).catch(()=>{}).finally(()=>setLoadingT(false))
  },[selThemes, period])
  useEffect(()=>{
    setLoadingM(true)
    fetch(`${API}/api/macro?period=${period}`)
      .then(r=>r.json()).then(d=>setMacroData(d.data)).catch(()=>{}).finally(()=>setLoadingM(false))
  },[period])
  const toggleTheme = (t) => setSelThemes(s=>s.includes(t)?s.filter(x=>x!==t):[...s,t])
  const toggleMacro = (t) => setSelMacro(s=>s.includes(t)?s.filter(x=>x!==t):[...s,t])
  const macroNames = Object.keys(macroData)
  return (
    <div style={{ padding:'28px 32px 48px' }}>
      <h1 style={{ fontSize:'24px', fontWeight:700, letterSpacing:'-0.02em', color:'#e8f0ff', marginBottom:'4px' }}>Compare</h1>
      <p style={{ fontSize:'12px', color:'var(--text3)', marginBottom:'20px' }}>Compare theme performance against each other and against macro indicators.</p>
      ...
      <div style={{ ...sHead, marginTop:'32px' }}><span style={sTitle}>Macro Comparison</span><div style={sLine}/></div>
      <div style={{ display:'flex', flexWrap:'wrap', gap:'6px', marginBottom:'16px' }}>
        {macroNames.map(t=>(
...
const selStyle = { background:'var(--bg3)', color:'var(--text)', border:'1px solid rgba(74,120,200,0.2)', borderRadius:'6px', fontFamily:'var(--font)', fontSize:'13px', padding:'6px 12px', cursor:'pointer', outline:'none', marginBottom:'4px' }
