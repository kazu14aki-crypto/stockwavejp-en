...
"""
# yfinance廃止: import yfinance as yf
import numpy as np
import warnings
import logging
...
def fetch_segment_detail(seg_name: str, period: str) -> list:
    ...
    for i, (name, ticker) in enumerate(stocks_def.items()):
        ...
        market_cap = 0
        # market_cap: InfowayのAPIで取得（yfinanceは廃止）
        # InfowayはAPIレスポンスのmarket_capフィールドから取得済みの場合を使用
        if market_cap == 0:
            try:
...
def fetch_macro_data(period: str) -> dict:
    ...
    for name, ticker in MACRO_TICKERS.items():
        ...
        if pdf is None or len(pdf) < 2:
            # 短期再取得フォールバック
            try:
                # yfinance廃止済み: Infoway APIへのフォールバックを実装
                pass  # InfowayからSparklineデータが取得できない場合はスキップ
            except Exception:
                pass
        if pdf is None or len(pdf) < 2: continue
        cl = pdf["Close"].dropna()
